...
public class GUI extends JFrame {
    ...
    //Very big class. Contains many very similar functions that I couldn't figure out 
    //how to automate much more with other functions.
    static String windowName;
    private static int attempts = 3;

    
    //Initializes some common variables as class-wide variables
    ...
    //GUI Main Page. Serves as the home page of the program.
    //Changes depending of the hasBanner flag. hasBanner = with a banner image
    public static void displayEventsGUI(boolean hasBanner) {
        ...
        for (Component component : pageParts) {
            ...
            if (component instanceof JLabel) {
                ...
                mainPanel.remove(component);        //Thorough wipe of the mainPanel 
            }
        }

        SwingUtilities.invokeLater(() -> {

            frameSetup();                           //Call Frame Setup function for size, colour, etc.

            mainPanel.removeAll();                  //Wipe again

            mainPanel.setLayout(new BorderLayout());            //Set layout, colour and add to frame. 
            mainPanel.setBackground(Color.decode(BG_COLOUR));
            frame.getContentPane().add(mainPanel);

            if (hasBanner) {

                // Banner Image added if hasBanner flag true
                ImageIcon bannerIcon = new ImageIcon("banner2.png");  //path to banner image
            ...
            textSubPanelWrapper.setBorder(BorderFactory.createEmptyBorder(10, 5, 10, 10)); //Panel padding
            ...
            if (hasBanner) {            //Adds textPanel differently depending if there is the 
                                        //banner image in the page or not.
            ...
            if (hasBanner) {
                ...
                displayEventsFromDB(textSubPanel, 3, false, false); //DisplayEvents
            ...
            //Add buttons (changes is there is a banner)
            if (hasBanner) {
            ...
            buttonPanelWrapper.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 5));
            buttonPanelWrapper.add(buttonPanel);

            mainPanel.add(buttonPanelWrapper, BorderLayout.WEST); //Wrap it all together

            frame.revalidate();
            frame.repaint();                    //Refresh page
    ...
    //Very similar to previous function. Serves as menu panel
...
//Very similar to previous function. Serves as menu panel
    ...
    public static void myEventsGUI(String username) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            // Wrapper Panel for textSubPanel
            JPanel textSubPanelWrapper = new JPanel(new BorderLayout());
            textSubPanelWrapper.setBorder(BorderFactory.createEmptyBorder(10, 5, 10, 10));
            textSubPanelWrapper.setBackground(Color.decode(TEXT_BG_COLOUR));
            ...
            JPanel buttonPanelWrapper = new JPanel(new BorderLayout());         //Lots of copy pasting from the previous functions
            buttonPanelWrapper.setBackground(Color.decode(TEXT_BG_COLOUR));
            buttonPanelWrapper.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 5));
            buttonPanelWrapper.add(buttonPanel);
    ...
    //Brings up the admin report info into a panel with a text area
    ...
    //Show admin details. Reads from passed in admin object.
    //Includes button to changes admin details
    ...
    //Getter for textSubPanel, one of the main global panels used
    public static JPanel getTextSubPanel() {
        return textSubPanel;
    }

    //Frame Setup done here to make things a bit easier
    ...
    //Can be called to add a button the fits in the side menu of the main windows
    //Buttons can be customised through parameters
    ...
    //Function that takes in a boring button and gives back a cool button
    //Give colour and hover colour to button. This hover effect works less well
    //as buttons get smaller
    private static void styleButton(JButton button, Color initialColour, Color hoverColour, boolean withBorder) {
        ...
        if (!withBorder) {
            ...
            button.addMouseListener(new java.awt.event.MouseAdapter() {
                ...
                @Override
                public void mouseEntered(java.awt.event.MouseEvent event) {  //Hover on
                ...
                @Override
                public void mouseExited(java.awt.event.MouseEvent event) {  //Hover off
    ...
    //Small function to add space on a panel
    private static void addGlue(JPanel panel) {
        panel.add(Box.createVerticalGlue());
    }

    //GUI Function to show the new window that comes up when a user, or admin goes to log in
    //Slightly changes how it runs based on the flags passed, and results in different
    //outcomes
    public static void displayLoginWindow(boolean isAdmin, boolean toEvents) {
        ...
        submitButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (isValidCredentials(username, password, isAdmin)) {          //Checks and validates login info
                    // Credentials are correct, perform actions based on isAdmin flag
                    loginFrame.dispose(); // Close the login window
                    onSuccessfulLogin(isAdmin, username, toEvents); //Call onSuccessfulLogin to continue login procedure
                } else {
                    attempts--;         //Log in fails counter
                    usernameField.setText("");
                    passwordField.setText("");
                    if (attempts > 0) {

                        errorPanel.repaint();           //Counts down attempts until you reach 0
                        ...
                        triesRemaining.setVisible(true);
                                                        ...
                                                        //When no more attempts left, you are moved to
                                                        //Main Menu and can't log in for 30s
        ...
        JButton cancelButton = new JButton("Cancel");           //Cancel buttin to close pop-up window
    ...
    //Similar to displayLoginWindow, shows a custom window over the main screen
    //with the searrch fields
    ...
    //Loads a list of events given from the database and displays them through calling 
    //other GUI making functions (createEventPanel, addComponentsToContentPanel, etc.)
    ...
    //Show all the users given from list
    ...
    //Show all the events given from list
    ...
    //Shows users matching search key 
    ...
    //Shows events matching search key
    ...
    //Very similar to earlier functions. Serves as menu panel
    public static void userManagementGUI() {
     ...
     //Very similar to previous functions. Serves as menu panel
    ...
    //Calls in gif for cool loading animation
    ...
    //Window to show admin details. This was really hard for some reason, I had
    //trouble with the padding
    ...
    //Function to make the top (or so) half of the window for events
    private static JPanel createEventTitlePanel(JPanel contentPanel, boolean all, boolean isAdmin) {
        ...
        if (!all && !isAdmin) {
            ...
            refreshButton.addActionListener(new ActionListener() {
                ...
                @Override
                public void actionPerformed(ActionEvent e) {            //Labels change depending on flags passed
    ...
    //Function to make the top (or so) half of the window for search results
    ...
    //Function to make the top (or so) half of the window for users
    ...
    //Function to create the panel that holds the event details and event image
    ...
    //Function to create the panel that holds the user details, used by admin
    ...
    //Similar to above function, used by user
    ...
    //Puzzles together pieces of the panel and returns them within a scrollPane
    ...
    //Checks if login details is correct. Begins login validation
    ...
    //Function to provide pop-up window to other classes
    ...
    //If login credentials check out, call this function to complete log in
    ...
    //GUI that opens when adding a new event
    ...
    //GUI that opens when adding a new user
    ...
    //GUI that opens when user is booking a new event
    ...
    //GUI that opens when deleting a user
    ...
    //GUI that opens when deleting an event
